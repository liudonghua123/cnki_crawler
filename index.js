...
async function completeInfo(inputData) {
  ...
  for (let i = 0; i < recordCount; i++) {
    ...
    spinner = ora({ text: `[${i + 1}/${recordCount}] Processing ${record.title} using cnki`, isEnabled: true }).start();
    // open www.cnki.net
    ...
    const authorsText = await page.$$eval(authorSpanSelector, els => els.map(el => el.innerText.replace(/\d/, '')));
    const releaseDateText = await page.$eval('div.top-first > div.top-tip > span > a:nth-child(2)', el => el.innerText);
...
(async () => {
  ...
  try {
    ...
    if (verbose) { console.info(chalk.blue.bold(`try to parse xlsx file: ${input}`)); }
    const inputData = xlsx.utils.sheet_to_json(worksheet, { raw: true });
    if (verbose) { console.info(chalk.blue.bold(`got parsed input data: ${JSON.stringify(inputData, null, 2)}`)); }
    // do the crawler job
    const results = await completeInfo(inputData);
    // output the result
    if (verbose) { console.info(chalk.blue.bold(`try to write xlsx result file: ${JSON.stringify(results, null, 2)}`)); }
